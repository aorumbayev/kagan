"""Service layer for GitHub plugin runtime orchestration."""
...
class GitHubPluginService:
    """Runtime orchestration service for bundled GitHub plugin operations."""
    ...
    async def connect_repo(self, params: dict[str, Any]) -> dict[str, Any]:
        """Connect a repository to GitHub with preflight checks."""
        project_id = params.get("project_id")
        repo_id = params.get("repo_id")

        resolved = await self._resolve_connect_target(project_id, repo_id)
        ...
        if existing_connection:
            ...
            if isinstance(raw_connection_data, dict) and raw_connection_data != connection_data:
                await self._upsert_repo_github_connection(repo.id, connection_data)
        ...
        await self._upsert_repo_github_connection(repo.id, connection_metadata)
    ...
    async def sync_issues(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        gh_path, gh_error = self._resolve_gh_cli_path()
        ...
        for issue in issues:
            ...
            if action == "insert":
                try:
                    ...
                    if update_fields:
                        await self._ctx.task_service.update_fields(task.id, **update_fields)
                ...
                if task_id:
                    try:
                        await self._ctx.task_service.update_fields(task_id, **changes)
                        result.add_outcome(
        ...
        await self._upsert_repo_sync_state(repo.id, new_checkpoint, new_mapping)
    ...
    async def acquire_lease(self, params: dict[str, Any]) -> dict[str, Any]:
    ...
    async def release_lease(self, params: dict[str, Any]) -> dict[str, Any]:
    ...
    async def get_lease_state(self, params: dict[str, Any]) -> dict[str, Any]:
    ...
    async def create_pr_for_task(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        await self._upsert_repo_pr_mapping(repo.id, pr_mapping)
    ...
    async def link_pr_to_task(self, params: dict[str, Any]) -> dict[str, Any]:
    ...
    async def reconcile_pr_status(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if pr_data.state == "MERGED":
            if task.status != TaskStatus.DONE:
                await self._ctx.task_service.update_fields(task_id, status=TaskStatus.DONE)
            ...
            if task.status != TaskStatus.DONE and task.status != TaskStatus.IN_PROGRESS:
                await self._ctx.task_service.update_fields(task_id, status=TaskStatus.IN_PROGRESS)
                task_status_changed = True
    ...
    async def _resolve_connect_target(
        ...
        project = await self._ctx.project_service.get_project(project_id)
        if not project:
            return {
                "success": False,
                "code": GH_PROJECT_REQUIRED,
        ...
        repos = await self._ctx.project_service.get_project_repos(project_id)
        if not repos:
            return {
                "success": False,
    ...
    async def _upsert_repo_github_connection(
        ...
        task_repo = self._ctx._task_repo
    ...
    async def _upsert_repo_pr_mapping(self, repo_id: str, pr_mapping: TaskPRMapping) -> None:
