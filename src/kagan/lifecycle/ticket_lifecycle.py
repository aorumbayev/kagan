...
class TicketLifecycle:
    ...
    async def delete_ticket(self, ticket: Ticket) -> tuple[bool, str]:
        ...
        try:
            ...
            await self.state.delete(ticket.id)
    ...
    async def merge_ticket(self, ticket: Ticket) -> tuple[bool, str]:
        ...
        if config.require_review_approval and ticket.checks_passed is not True:
            ...
            await self.state.append_event(ticket.id, "policy", message)
        ...
        async def _do_merge() -> tuple[bool, str]:
            ...
            if success:
                ...
                await self.state.append_event(ticket.id, "merge", f"Merged to {base}")
            else:
                ...
                if _is_merge_conflict(message):
                    ...
                    await self.state.append_event(ticket.id, "merge", f"Merge failed: {message}")
    ...
    async def apply_rejection_feedback(
        ...
        if feedback:
            ...
            await self.state.append_event(ticket.id, "review", "Rejected")
        ...
        refreshed_ticket = await self.state.get(ticket.id)
    ...
    async def get_review_ticket(self, ticket: Ticket) -> Ticket | None:
        ...
        all_tickets = await self.state.get_all()
