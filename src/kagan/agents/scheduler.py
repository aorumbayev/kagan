...
class Scheduler:
    ...
    def __init__(
        self,
        state_manager: TicketRepository,
    ...
    async def handle_status_change(
        ...
        Called by TicketRepository when ticket status changes.
    ...
    async def _check_waiting_tickets(self) -> None:
        ...
        # Get all tickets
        tickets = await self._state.get_all()
        for ticket in tickets:
            if (
    ...
    def _notify_error(self, ticket_id: str, message: str) -> None:
        ...
        _ = asyncio.create_task(self._state.update_fields(ticket_id, last_error=message[:500]))
    ...
    async def _auto_merge(self, ticket: Ticket) -> None:
        ...
        async with self._merge_lock:
            ...
            if success:
                ...
                await self._state.append_event(ticket.id, "merge", f"Auto-merged to {base}")
            else:
    ...
    async def _update_ticket_status(self, ticket_id: str, status: TicketStatus) -> None:
        ...
        await self._state.update_fields(ticket_id, status=status)
    ...
    async def _handle_complete(self, ticket: Ticket) -> None:
        ...
        await self._state.update_fields(ticket.id, **review_updates)
        self._notify_ticket_changed()
        review_event = "Review passed" if checks_passed else f"Review failed: {review_summary}"
        await self._state.append_event(ticket.id, "review", review_event[:200])
