...
class TestCreateTicket:
    ...
    async def test_create_pair_ticket_in_backlog(self, state_manager: TicketRepository):
        ...
        created = await state_manager.create(ticket)
    ...
    async def test_create_auto_ticket_in_backlog(self, state_manager: TicketRepository):
    ...
    async def test_create_ticket_with_acceptance_criteria(self, state_manager: TicketRepository):
...
class TestReadTicket:
    ...
    async def test_get_ticket_by_id(self, state_manager: TicketRepository):
    ...
    async def test_get_nonexistent_ticket_returns_none(self, state_manager: TicketRepository):
        """Fetching nonexistent ticket returns None."""
        result = await state_manager.get("nonexistent-id")
        assert result is None

    async def test_list_tickets_by_status(self, state_manager: TicketRepository):
        ...
        await state_manager.create(backlog)
        await state_manager.create(in_progress)

        backlog_tickets = await state_manager.get_by_status(TicketStatus.BACKLOG)
        wip_tickets = await state_manager.get_by_status(TicketStatus.IN_PROGRESS)
...
class TestUpdateTicket:
    ...
    async def test_update_ticket_title(self, state_manager: TicketRepository):
        """Updating title persists the change."""
        ticket = Ticket.create(title="Original title")
        await state_manager.create(ticket)

        updated = await state_manager.update_fields(ticket.id, title="Updated title")

        assert updated is not None
        assert updated.title == "Updated title"

    async def test_update_ticket_priority(self, state_manager: TicketRepository):
        """Updating priority changes ordering behavior."""
        ticket = Ticket.create(title="Test", priority=TicketPriority.LOW)
        await state_manager.create(ticket)

        updated = await state_manager.update_fields(ticket.id, priority=TicketPriority.HIGH)

        assert updated is not None
        assert updated.priority == TicketPriority.HIGH

    async def test_update_multiple_fields(self, state_manager: TicketRepository):
        ...
        updated = await state_manager.update_fields(
            ticket.id,
            title="New title",
...
class TestDeleteTicket:
    ...
    async def test_delete_ticket_removes_from_database(self, state_manager: TicketRepository):
        """Deleting ticket removes it from database."""
        ticket = Ticket.create(title="To be deleted")
        await state_manager.create(ticket)

        result = await state_manager.delete(ticket.id)

        assert result is True
        assert await state_manager.get(ticket.id) is None

    async def test_delete_nonexistent_ticket_returns_false(self, state_manager: TicketRepository):
        """Deleting nonexistent ticket returns False."""
        result = await state_manager.delete("nonexistent-id")
        assert result is False

    async def test_delete_with_lifecycle_cleanup(
...
class TestStatusTransitions:
    ...
    async def test_backlog_to_in_progress(self, state_manager: TicketRepository):
        """Moving from BACKLOG to IN_PROGRESS is valid."""
        ticket = Ticket.create(title="Start work", status=TicketStatus.BACKLOG)
        await state_manager.create(ticket)

        updated = await state_manager.move(ticket.id, TicketStatus.IN_PROGRESS)

        assert updated is not None
        assert updated.status == TicketStatus.IN_PROGRESS

    async def test_in_progress_to_review(self, state_manager: TicketRepository):
        """Moving from IN_PROGRESS to REVIEW is valid."""
        ticket = Ticket.create(title="Ready for review", status=TicketStatus.IN_PROGRESS)
        await state_manager.create(ticket)

        updated = await state_manager.move(ticket.id, TicketStatus.REVIEW)

        assert updated is not None
        assert updated.status == TicketStatus.REVIEW

    async def test_review_to_done(self, state_manager: TicketRepository):
        """Moving from REVIEW to DONE is valid (typically via merge)."""
        ticket = Ticket.create(title="Approved", status=TicketStatus.REVIEW)
        await state_manager.create(ticket)

        updated = await state_manager.move(ticket.id, TicketStatus.DONE)

        assert updated is not None
        assert updated.status == TicketStatus.DONE

    async def test_move_backward_review_to_in_progress(self, state_manager: TicketRepository):
    ...
    async def test_status_change_callback_triggered(self, state_manager: TicketRepository):
...
class TestTicketTypes:
    ...
    async def test_auto_ticket_has_iteration_tracking(self, state_manager: TicketRepository):
    ...
    async def test_pair_ticket_tracks_session_active(self, state_manager: TicketRepository):
...
class TestReviewWorkflow:
    ...
    async def test_set_review_summary_and_checks(self, state_manager: TicketRepository):
    ...
    async def test_review_rejection_updates_checks(self, state_manager: TicketRepository):
...
class TestRejectionHandling:
    ...
    async def test_rejection_without_feedback(
        self, state_manager: TicketRepository, git_repo: Path
    ):
        ...
        events = await state_manager.get_events(ticket.id)
...
class TestScratchpadPersistence:
    ...
    async def test_scratchpad_create_and_read(self, state_manager: TicketRepository):
    ...
    async def test_scratchpad_update_replaces(self, state_manager: TicketRepository):
    ...
    async def test_scratchpad_empty_by_default(self, state_manager: TicketRepository):
...
class TestTicketEvents:
    ...
    async def test_append_and_get_events(self, state_manager: TicketRepository):
        """Events can be appended and retrieved."""
        ticket = Ticket.create(title="Event tracking")
        await state_manager.create(ticket)

        await state_manager.append_event(ticket.id, "merge", "Merged to main")
        await state_manager.append_event(ticket.id, "review", "Review passed")
    ...
    async def test_events_limited_by_default(self, state_manager: TicketRepository):
        """get_ticket_events respects limit parameter."""
        ticket = Ticket.create(title="Many events")
        await state_manager.create(ticket)

        for i in range(25):
            await state_manager.append_event(ticket.id, "test", f"Event {i}")

        events = await state_manager.get_events(ticket.id, limit=10)
...
class TestTicketSearch:
    ...
    async def test_search_by_title(self, state_manager: TicketRepository):
        ...
        await state_manager.create(ticket1)
        await state_manager.create(ticket2)

        results = await state_manager.search("authentication")

        assert len(results) == 1
        assert results[0].title == "Fix authentication bug"

    async def test_search_by_description(self, state_manager: TicketRepository):
        ...
        results = await state_manager.search("OAuth2")

        assert len(results) == 1
        assert results[0].id == ticket.id

    async def test_search_empty_query_returns_empty(self, state_manager: TicketRepository):
